...
class LLMCallRecorder:
    ...
    def _record(
        self, label, duration, response_json, failed=False, error=None
    ):
    ...
    def _wrapped_requests_post(self, url, *args, **kwargs):
        ...
        try:
            ...
            self._record(
                self._label_for(prompt), duration, {},
    ...
    def _wrapped_session_post(self, session_self, url, *args, **kwargs):
        ...
        try:
            response = self._orig_session_post(
                session_self, url, *args, **kwargs
            )
        except Exception as e:
            duration = time.perf_counter() - start
            self._record(
    ...
    def __enter__(self):
        ...
        def session_post_patch(session_self, url, *args, **kwargs):
            return recorder._wrapped_session_post(
                session_self, url, *args, **kwargs
            )

        requests.Session.post = session_post_patch
                ...
                if call.get("failed"):
                    self.stdout.write(
                        self.style.ERROR(
                            ...
                            f"FAILED after {call['duration']:.2f}s -- "
                            f"{call['error']}"
                        )
                    )
                    continue
                ...
                if c.get("failed"):
                    self.stdout.write(
                        self.style.ERROR(
                            ...
                            f"FAILED after {c['duration']:.2f}s -- "
                            f"{c['error']}"
                        )
                    )
                    continue
