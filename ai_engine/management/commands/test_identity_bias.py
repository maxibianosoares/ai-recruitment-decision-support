...
class Command(BaseCommand):
    ...
    def _print_report(self, rows):
        ...
        if not rows:
            ...
            self.stderr.write(
                self.style.ERROR(
                    "\n[NO DATA] Every pipeline run failed before "
                    "producing a result -- see the 'Pipeline failed "
                    "for ...' lines above for the actual error. No "
                    "bias/reliability conclusion can be drawn from "
                    "zero successful runs. Fix the underlying error "
                    "and re-run before treating any output below as "
                    "evidence."
                )
            )
